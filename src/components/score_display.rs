...
use dioxus::prelude::*;
use crate::components::calculation::recommendation_calculation::{FinishRecValue, NonFinishRecValue, RecValue};
...
fn FinishRecommendationRow(finish_rec_value: FinishRecValue, row_nr: u16) -> Element {
    ...
    rsx! {
        ...
        if row_nr > 0 {
                                        ...
                                        if secondary_rec.as_ref().is_some() {
                                            p {
                                                "{primary_rec} | {secondary_rec.as_ref().unwrap()}"}
                                        }
        }
    }
}
